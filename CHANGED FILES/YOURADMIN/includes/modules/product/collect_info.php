 ...
 * @version $Id: collect_info.php for pdf attachment 2017-04-12 07:24:50Z webchills $
 */
...
// set image delete
  ...
  // set pdf attachment overwrite
  $on_overwrite_pdf_attachment = true;
  $off_overwrite_pdf_attachment = false;
// set pdf attachment delete
  $on_pdf_attachment_delete = false;
  $off_pdf_attachment_delete = true;
?>
...
?>
          <tr>
            <td class="main"><?php if ($i == 0) echo TEXT_PDF_ATTACHMENT_NAME; ?></td>
            <td class="main"><?php echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['pdf_attachment_name']) . '&nbsp;' . zen_draw_input_field('pdf_attachment_name[' . $languages[$i]['id'] . ']', htmlspecialchars(isset($pdf_attachment_name[$languages[$i]['id']]) ? stripslashes($pdf_attachment_name[$languages[$i]['id']]) : zen_get_pdf_attachment_name($pInfo->products_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE), zen_set_field_length(TABLE_PRODUCTS_DESCRIPTION, 'pdf_attachment_name')); ?></td>
          </tr>
<?php
    }
?>
              <tr>
                <td class="main"><?php echo TEXT_PRODUCTS_PDF_ATTACHMENT; ?></td>
                <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_file_field('products_pdf_attachment') . '&nbsp;' . ($pInfo->products_pdf_attachment !='' ? TEXT_PDF_ATTACHMENT_CURRENT . $pInfo->products_pdf_attachment : TEXT_PDF_ATTACHMENT_CURRENT . '&nbsp;' . NONE) . zen_draw_hidden_field('products_previous_pdf_attachment', $pInfo->products_pdf_attachment); ?></td>
                <td valign = "center" class="main"><?php echo TEXT_PRODUCTS_PDF_ATTACHMENT_DIR; ?>&nbsp;<?php echo zen_draw_pull_down_menu('pdf_attachment_dir', $dir_attach_info, $default_attach_directory); ?></td>
						  </tr>
              <tr>
                <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15'); ?></td>
                <td class="main" valign="top"><?php echo TEXT_PDF_ATTACHMENT_DELETE . ' ' . zen_draw_radio_field('pdf_attachment_delete', '0', $off_pdf_attachment_delete) . '&nbsp;' . TABLE_HEADING_NO . ' ' . zen_draw_radio_field('pdf_attachment_delete', '1', $on_pdf_attachment_delete) . '&nbsp;' . TABLE_HEADING_YES; ?></td>
	  	    	  </tr>

              <tr>
                <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15'); ?></td>
                <td colspan="3" class="main" valign="top"><?php echo TEXT_PDF_ATTACHMENT_OVERWRITE  . ' ' . zen_draw_radio_field('overwrite_pdf_attachment', '0', $off_overwrite_pdf_attachment) . '&nbsp;' . TABLE_HEADING_NO . ' ' . zen_draw_radio_field('overwrite_pdf_attachment', '1', $on_overwrite_pdf_attachment) . '&nbsp;' . TABLE_HEADING_YES; ?>
                  <?php echo '<br />' . TEXT_PRODUCTS_PDF_ATTACHMENT_MANUAL . '&nbsp;' . zen_draw_input_field('products_pdf_attachment_manual'); ?></td>
              </tr>
            </table></td>
          </tr>
<tr>
