 ...
 * @version $Id: copy_to_confirm.php for pdf attachment 2017-04-12 07:25:16Z webchills $
 */
...
// Copy attributes to duplicate product
          ...
          if ($_POST['copy_as'] == 'link') {
            ...
            $db->Execute("insert into " . TABLE_PRODUCTS . "
                                      (products_type, products_quantity, products_model, products_image, products_pdf_attachment, 
            ...
            $description = $db->Execute("select language_id, products_name, pdf_attachment_name, products_description,
                                                             products_url
                                         from " . TABLE_PRODUCTS_DESCRIPTION . "
                                         where products_id = '" . (int)$products_id . "'");
            while (!$description->EOF) {
              $db->Execute("insert into " . TABLE_PRODUCTS_DESCRIPTION . "
                                        (products_id, language_id, products_name, pdf_attachment_name, products_description,
                                         products_url, products_viewed)
                            values ('" . (int)$dup_products_id . "',
                                    '" . (int)$description->fields['language_id'] . "',
                                    '" . zen_db_input($description->fields['products_name']) . "',
                                    '" . zen_db_input($description->fields['pdf_attachment_name']) . "',
